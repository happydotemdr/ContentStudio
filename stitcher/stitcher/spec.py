...
Spec §3 also requires a JSON Schema generated from these models, so external
tools can validate a render-spec.json without importing Python. It is
generated at the bottom of this module and committed at
`stitcher/schema/render-spec.schema.json`; regenerate it with
`cd stitcher && python -m stitcher.spec`. See `render_spec_schema` for why
the file is committed rather than produced on demand, and tests/test_spec.py
for the drift test that stops it rotting -- which fires on a pydantic upgrade
as well as on a model edit, and regenerating is the right answer to both.
"""
...
# tools can validate without importing Python." The path is spec-literal,
# relative to the repo root: this module is stitcher/stitcher/spec.py, so
# parents[1] is the stitcher/ project directory and the file lands beside
# the package and tests/ rather than inside the package.
SCHEMA_PATH = Path(__file__).resolve().parents[1] / "schema" / "render-spec.schema.json"

_SCHEMA_COMMENT = (
    "Generated from the pydantic models in stitcher/stitcher/spec.py (design "
    "spec section 3). Do not edit by hand: run `cd stitcher && python -m "
    "stitcher.spec` to regenerate. tests/test_spec.py fails if this file "
    "drifts from the models, or if an upgraded pydantic changes how it emits "
    "schemas; regenerating is the fix in both cases."
)


def render_spec_schema() -> dict:
    """The JSON Schema for render-spec.json, generated from RenderSpec.

    `by_alias=True` is passed explicitly even though it is pydantic's current
    default: this file describes render-spec.json AS WRITTEN ON DISK, whose
    keys are `in`/`out`, not the Python attributes `start`/`end`. A future
    default change must not be able to silently publish the Python names to
    external validators.

    `mode="validation"` for the same reason: external tools validate INPUT
    documents, and the serialization schema would describe what this module
    dumps rather than what it accepts.
    """
    schema = RenderSpec.model_json_schema(by_alias=True, mode="validation")
    return {
...
def schema_json() -> str:
    """Exactly the bytes SCHEMA_PATH holds: sorted keys, 2-space, trailing LF.

    Sorted and fixed-format so a regeneration only ever produces a diff when
    the MODELS changed, never because a dict happened to iterate differently.
    """
    return json.dumps(render_spec_schema(), indent=2, sort_keys=True) + "\n"


def write_schema(path: Path | None = None) -> Path:
    """Write the generated schema. Committed artifact, regenerated by hand.

    The alternative -- generating on demand at import or CLI time -- was
    rejected: §3's whole point is that an external tool can validate WITHOUT
    importing Python, which a file that only exists after a Python run does
    not deliver. A committed file rots instead, so the drift test in
    tests/test_spec.py fails the suite the moment the models and the file
    disagree, and names `cd stitcher && python -m stitcher.spec` as the fix.

    That test also fires when nobody touched a model: requirements.txt pins
    floors (R2), so a pydantic upgrade that changes schema emission moves the
    generated bytes on its own. Regenerating is still the fix -- the committed
    file is what external validators read, and it has to describe THIS build.
    """
    target = SCHEMA_PATH if path is None else path
    target.parent.mkdir(parents=True, exist_ok=True)
    # newline="\n" so the committed file is LF on every platform and the
    # drift comparison never turns into a line-ending argument.
