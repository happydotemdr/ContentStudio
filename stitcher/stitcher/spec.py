...
`python -m stitcher.spec`. See `render_spec_schema` for why the file is
committed rather than produced on demand, and tests/test_spec.py for the
drift test that stops it rotting.
"""
...
_SCHEMA_COMMENT = (
    ...
    "spec section 3). Do not edit by hand: run `python -m stitcher.spec` to "
    "regenerate. tests/test_spec.py fails if this file drifts from the models."
)
...
def write_schema(path: Path | None = None) -> Path:
    ...
    disagree, and names this command as the fix.
    """
