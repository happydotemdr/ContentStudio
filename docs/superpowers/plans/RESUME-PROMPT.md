...
Start P13 in a fresh worktree off `origin/main` via `superpowers:using-git-worktrees` — do not reuse P15's worktree/branch (`worktree-worktree-p15-ui`, merged and gone) or this session's own docs-followup worktree (`worktree-p15-docs-followup`, also merged and gone by the time you read this). `origin/main` at P15's merge commit `8893789` (PR #54) already contains everything P15 landed, plus this session's own docs-only follow-up commits (P15's plan §8 Outcome, the master plan's wave-table/contract/baseline updates, P13's own §0 pre-flight amendment, and this file) via their own separate PR — check `gh pr list --state merged --limit 5` if you need that PR's number or merge SHA specifically; it isn't load-bearing for anything below, just confirm those commits are present with `git log --oneline -10 origin/main` before assuming this prompt's own claims about P15's outcome are current.
...
- A separate, unrelated, local-only failure predates this programme and is orthogonal to it: `tests/test_build_cowork_plugin.py`'s two staleness checks against `.claude/skills/` — fixed by `bash scripts/build-cowork-plugin.sh` if the operator wants it fixed. **This one is directly relevant to P13 in a way it wasn't for prior packages**, since P13 edits `.claude/skills/**` extensively — expect this staleness check to go (further) red mid-package and stay that way until P13 finishes; do not treat it as a P13 regression, and do not feel obligated to run the build script mid-package unless the plan says to.
...
**Suite:** root suite only (`python -m pytest tests/ -q`) — P13 touches no app-suite files. This is the **first** package in the programme whose tests live entirely in the root suite rather than `pipeline-app/tests/`; do not reflexively `cd pipeline-app` before running P13's tests.
...
**Nothing else in this plan's 48 findings, 18 tasks, or the P14 contract (§6.1) shows any sign of drift.**
...
- Once both suites are fully green, there is no longer a documented baseline to distinguish "the same old failures" from "a new regression" (except the one local-only Cowork-plugin staleness noted above — and see this prompt's own note above about why that one is directly relevant to P13, not just orthogonal noise).
...
Both suites are fully green with no documented exceptions beyond P15's own 2 documented xfail — root suite 445 passed/1 skipped/0 failed; app suite 1954 passed/4 skipped/2 xfailed/0 failed, as of P15's merge (`8893789`). Track any future failure as real (except the one local-only Cowork-plugin staleness noted above, orthogonal to the programme but directly relevant to P13's own scope — see the baseline section above).
