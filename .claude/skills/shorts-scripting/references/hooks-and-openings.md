...
Distilled from `docs/headless-youtube-audit.md` §4 ("Hooks & openings") and
`docs/headless-shorts-production-playbook.md` §2 ("The Hook (0–3s)"). This is the
0–3s beat — the single highest-leverage seconds in the whole script.

## The window is brutal, and it is 0–3s, not 0–15s

- **On Shorts specifically, 50–60% of viewers who leave do so within the first 3 `[C]`
...
- **Cut the intro entirely; deliver value in the first seconds — strongly-supported `[C]`
...
- **Never open with "in this video I'm going to show you" or treat the top as an `[C]`
  intro/summary** `[C] (vidIQ, UCrC5B3Soyc; Nick Nimmin, 2vkX1X1K3WM)`.

## The hook must pay off the packaging promise

- **The first seconds must deliver on the title/thumbnail promise — strongly- `[C]`
  ...
  6RSWJ0IxKvs; Nick Nimmin, kcSOFqJhR9I)`. Match the opening's emotion/energy to
  how the packaging framed it `[C] (Nick Nimmin, kcSOFqJhR9I)`.
- If you don't have the concept brief's packaging direction (title/cover frame) in
  hand when writing the hook line, stop and ask for it — writing a hook that
  doesn't pay off the packaging is exactly the failure mode above.
...
- **Address a singular "you," not "hey guys."** `[C] (Nick Nimmin, IF-PD6XMjYY)`
...
- **Make the first frame a visual hook that raises a question before a word is `[C]`
  ...
  frame one)`.
- **Align the three hook layers — what's shown, what's written on screen, what's `[C]`
...
## Named intro formulas (long-form-sourced, use as a structural skeleton)

These two formulas come from long-form videos, not Shorts specifically. `[I]`
Compress them into the Hook+Setup beats rather than trying to fit all five
...
- **One Person Business's four-part intro: click confirmation → common belief → `[C]`
...
The corpus is explicit that **Jenny Hoyos's findings are mostly *observed
structural technique* reverse-engineered from her exemplar Shorts (medium
confidence), not stated advice** `(docs/README.md; docs/headless-youtube-audit.md
§ intro)`. Carry that caveat through verbatim any time one of these is used —
don't present it with the same confidence as a stated, multi-channel rule:

- **Provocative core-premise question** as the hook `[C] (Jenny Hoyos,
  BJv4MYm7-rU)` — medium confidence.
- **In-medias-res drop into a high-stakes moment** `[C] (Jenny Hoyos,
...
- **Anchor the whole Short on a single clear premise plus one explicit `[C]`
...
There is no corpus-sourced guidance on genre-specific hook phrasing (e.g., how a
finance Short's hook should differ from a comedy Short's). If a concept brief
needs that kind of tailoring, say so explicitly and write the hook from the
general rules above rather than inventing a genre convention.
