# Endings, payoff, and the CTA/comment-bait line

Distilled from `docs/headless-youtube-audit.md` §4 ("Endings") and
`docs/headless-shorts-production-playbook.md` §2 ("Seamless loop-to-start"). This
covers the Payoff and Loop/CTA beats.

## Never signal "the video is over"

- **`[C]` Never end with "thanks for watching, like and subscribe" — bridge to a
...
- **Don't use finalizing language** `[C]` ("that's how you…", "to recap", "our last
  ...
  ask for the close.
- **`[C]` Ask a specific question to earn comments, instead of "leave a comment
...
- **`[C]` Ask the subscribe about one-third in, after delivering value, framed as a
  ...
  Short at all. Scale the *principle* — ask after value has landed, not before
  — rather than the literal timestamp: on a Short that means folding any
  subscribe ask into the Payoff or Loop/CTA beat, never the Hook or Setup —
  this Shorts-scale mapping is this skill's own synthesis.
...
- **`[C]` Low unique-viewers-with-high-views signals the video is being rewatched
  (looped)** — the outcome a mirrored ending is built to produce `[C] (Nate
  Black, IHDJkJpYC90)`.
- **`[C]` "Good abandonment" — a satisfied viewer who got the answer and left early
...
There's no corpus-sourced rule on how many comment-bait questions to ask, or on
alternating question styles across a batch of Shorts. Write one specific,
premise-tied question per script and don't invent a rotation scheme.
