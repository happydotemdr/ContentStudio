...
## 3. Step 2.5 — the world lock, read from the styleboard artifact

`shorts-styleboard` already ran against this script and produced the styleboard artifact
below — this is its output, reproduced here verbatim (`tests/fixtures/worked_example_styleboard.md`),
not something this skill builds. Step 2.5's job is to read it `[I]`:

```
=== STYLEBOARD — letkidsplay (Plutarch / club soccer) ===
...
named here with that one-line rationale rather than picked for its visuals alone `[I]`.
...
- **Register A slot (`{style:register_a}`):** binds to `rgs-present-soccer-a`, harvested for this
  Short specifically (present-day club-soccer look).
- **Register B slot (`{style:register_b}`):** binds to `rgs-sourceera-painterly-b`, the channel's
  fixed painterly signature, harvested once and reused unchanged on every Short.

No `--oref`/`{char:...}` on either register: Register A has no recurring named character to lock a
likeness for, and Register B's figure treatment is archetype-only by contract (unnamed, face
averted or shadowed, dressed to the role) — there is no likeness to lock, and adding one would force
the whole job into V7 at 2× GPU cost for no visual gain `[T] (verified 2026-07-26)`.
...
```text
documentary sports photography, a club soccer coach crouching beside a line of cones handing a water bottle to a waiting player, a corner flag catching a gust just behind them, cleats and shin guards scattered on the painted touchline nearby, a stack of folded practice bibs set on the grass to one side, eye-level three-quarter view from pitch side, 50mm lens at f4, moderate depth holding both figures and the cones legible, a thin shaft of morning sun breaking through low cloud, palette warming from cold slate toward pale gold, No Text. --ar 9:16 --raw --s 105 {style:register_a}
```
...
- **Gate C** (`scripts/lint_prompt_sheet.py`): run against the sheet exactly as emitted above
  (extracted verbatim into `tests/fixtures/worked_example_sheet.md`), against the styleboard
  artifact from §3 (`tests/fixtures/worked_example_styleboard.md`):
...
WHOLE-SHORT SETUP
  --ar 9:16
  Register A style: {style:register_a}   [resolved against tests/fixtures/worked_example_styleboard.md]
  Register B style: {style:register_b}   [resolved against tests/fixtures/worked_example_styleboard.md]
  Styleboard:        tests/fixtures/worked_example_styleboard.md
...
VALIDATION
  ...
  Gate C: pass, 11 shots, 0 findings — `python scripts/lint_prompt_sheet.py
          tests/fixtures/worked_example_sheet.md --styleboard tests/fixtures/worked_example_styleboard.md`
```

The `{style:register_a}` / `{style:register_b}` tokens above are slots, not literal codes — they
resolve against the styleboard's `slot_register_a: rgs-present-soccer-a` / `slot_register_b:
rgs-sourceera-painterly-b` bindings (§3) at generate time. This is a worked example, not a
production run, so no real render-console resolution happened here.
