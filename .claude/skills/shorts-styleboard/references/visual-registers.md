...
- **World lock.** One sport per Short, named in every Register A prompt; at least one signature object from the world-lock block must be in frame; the strings `empty gym` and `empty youth gym` are banned — a present-day beat with no signature object and no human presence reads as a stock-photo void, not a specific world `[I]`.
...
- **Consistency.** One `--sref` code harvested per Short and reused across every Register A prompt in that Short `[I]`.
