...
def test_main_returns_nonzero_when_a_result_is_publish_ok_notify_failed(tmp_path, monkeypatch):
    # orchestrator.process_candidate returns this literal string when the
    # draft published but the notify email failed (Finding 2) -- it's a
    # real per-wake failure, not an "error: " result, so main() must still
    # count it toward a nonzero exit code or the cron's own exit status
    # goes silently green while notify keeps failing.
    def fake_run_once(conn, doc_ingest_conn, calendar_service, gmail_service, drive_service, cfg, now_utc):
        return ["published: acme", "publish_ok_notify_failed", "skipped: gamma"]
